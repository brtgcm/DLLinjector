...
{
    private static string DllPath = "your_path";
    ...
    public static bool Run(string DllPath)
    {
        ...
        {
            throw new ApplicationException("The process was not found.");
        }

        hProcess = OpenProcess(ProcessAccessFlags.All, false, (int)pid);
    ...
    public static UInt32 GetProcessPID()
    {
        IntPtr hwProc = (IntPtr)FindWindow(null, ""); // put the name of the window to get the PID

        if (hwProc == IntPtr.Zero)
        {
            return 0;
        }

        UInt32 ret = UInt32.MinValue;
        GetWindowThreadProcessId(hwProc, out ret);
    ...
    private static bool Unhook(string methodName, string dllName, Int32 index)
    {
        ...
        byte[] originalProcessBytes = new byte[6];

        ReadProcessMemory(hProcess, originalMethodAddress, originalProcessBytes, sizeof(byte) * 6, out _);

        for (int i = 0; i < originalProcessBytes.Length; i++)
        {
            originalBytes[index, i] = originalProcessBytes[i];
        }
        ...
        return WriteProcessMemory(hProcess, originalMethodAddress, originalDLLBytes, sizeof(byte) * 6, out _);
    }
    ...
    private static bool RestoreHook(string methodName, string dllName, Int32 index)
    {
        ...
        return WriteProcessMemory(hProcess, originalMethodAdress, origBytes, sizeof(byte) * 6, out _);
    }
    
    private static string GetPathDLL()
    {
        return Program.DllPath;
    }
    ...
    static void Main(String[] args)
    {
        ...
        try
        {
            if(Program.run(GetPathDLL()))
            {
                Console.WriteLine("[#] DLL injected!");
            }
            else
            {
                Console.WriteLine("[X] Error, DLL not injected");   
            }
        }catch(Exception ex)
        {}
    }
